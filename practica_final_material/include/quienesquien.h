...
public:
	...
	*/
	...
	void nodosfr(bintree<Pregunta>::node nodo);  
	int mejor_atributo(vector<bool> atrib_sin_uso, vector<bool> personajes_sin_uso, int n);
	int personajes_con_atributo(vector<bool> personajes_sin_uso, int atr);
	void usar_arbol(bintree<Pregunta> arbol_nuevo);
	void mod_node(const bintree<Pregunta>::node &nodo, Pregunta &pregunta, const int pos, bool lado);
	int devolver_pos_atr(string atr);
	int devolder_pos_per(string per);
